...
class FactCheckLib:
    def __init__(self, language=None, num_results=100, csv_filename=None):
    ...
    def convert_to_csv(self, data, query):
        if not data:
            print("No data to save.")
            return
        csv_filename =  f"{self._sanitize_query_for_filename(query)}.csv"
        with open(self.filepath+csv_filename, mode='w', newline='', encoding='utf-8') as file:
